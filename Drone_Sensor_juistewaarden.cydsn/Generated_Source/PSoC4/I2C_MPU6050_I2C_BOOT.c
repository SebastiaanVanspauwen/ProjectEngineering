...
*
* \globalvars
*  I2C_MPU6050_applyBuffer - the flag to release the buffer with a response
...
*
* \globalvars
*  I2C_MPU6050_slRdBufSize - used to store slave read buffer size.
*  I2C_MPU6050_slRdBufIndex - used to store the current index within the
*  slave read buffer.
*  I2C_MPU6050_slWrBufIndex - used to store current index within slave
*  write buffer.
*  I2C_MPU6050_slStatus  - used to store current status of I2C slave.
*
*******************************************************************************/
            ...
            if (0u != (I2C_MPU6050_I2C_SSTAT_WR_CMPLT & I2C_MPU6050_slStatus))
            {
                ...
                *count = (uint16) I2C_MPU6050_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) I2C_MPU6050_slWriteBuf,
                                              I2C_MPU6050_BYTES_TO_COPY(*count, size));
...
*
* \globalvars
*  I2C_MPU6050_applyBuffer - the flag to release the buffer with
*  to be read by the host.
*
*******************************************************************************/
...
static void I2C_MPU6050_I2CResposeInsert(void)
{
    if (I2C_MPU6050_CHECK_INTR_SLAVE_MASKED(I2C_MPU6050_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        ...
        else
        {
            ...
            if (I2C_MPU6050_slRdBufIndex != I2C_MPU6050_slRdBufSize)
            {
                I2C_MPU6050_slRdBufIndex = I2C_MPU6050_slRdBufSize;
            }
        }
    }
}
