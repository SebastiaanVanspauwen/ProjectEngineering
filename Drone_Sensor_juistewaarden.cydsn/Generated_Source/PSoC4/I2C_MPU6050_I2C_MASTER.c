...
/* Master variables */
volatile uint16 I2C_MPU6050_mstrStatus;      /* Master Status byte  */
volatile uint8  I2C_MPU6050_mstrControl;     /* Master Control byte */

/* Receive buffer variables */
volatile uint8 * I2C_MPU6050_mstrRdBufPtr;   /* Pointer to Master Read buffer */
volatile uint32  I2C_MPU6050_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  I2C_MPU6050_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * I2C_MPU6050_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  I2C_MPU6050_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  I2C_MPU6050_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  I2C_MPU6050_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!I2C_MPU6050_CY_SCBIP_V0 && \
    I2C_MPU6050_I2C_MULTI_MASTER_SLAVE_CONST && I2C_MPU6050_I2C_WAKE_ENABLE_CONST)
    static void I2C_MPU6050_I2CMasterDisableEcAm(void);
#else
    #define I2C_MPU6050_I2CMasterDisableEcAm()     do{ /* Empty */ }while(0)
#endif /* (!I2C_MPU6050_CY_SCBIP_V0) */

static uint32 I2C_MPU6050_I2CMasterHandleStatus(uint32 status);
static uint32 I2C_MPU6050_I2CMasterWaitOneUnit(uint32 *timeout);


/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterWriteBuf
****************************************************************************//**
*
...
*  Enables the I2C interrupt and clears I2C_MPU6050_I2C_MSTAT_WR_CMPLT
*  status.
*
...
*  - I2C_MPU6050_I2C_MODE_COMPLETE_XFER - Perform complete transfer
*    from Start to Stop.
*  - I2C_MPU6050_I2C_MODE_REPEAT_START - Send Repeat Start instead
*    of Start. A Stop is generated after transfer is completed unless
*    NO_STOP is specified.
*  - I2C_MPU6050_I2C_MODE_NO_STOP Execute transfer without a Stop.
*    The following transfer expected to perform ReStart.
*
* \return
*  Error status.
*  - I2C_MPU6050_I2C_MSTR_NO_ERROR - Function complete without error.
*    The master started the transfer.
*  - I2C_MPU6050_I2C_MSTR_BUS_BUSY - Bus is busy. Nothing was sent on
*    the bus. The attempt has to be retried.
*  - I2C_MPU6050_I2C_MSTR_NOT_READY - Master is not ready for to start
...
*
* \globalvars
*  I2C_MPU6050_mstrStatus  - used to store current status of I2C Master.
*  I2C_MPU6050_state       - used to store current state of software FSM.
*  I2C_MPU6050_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  I2C_MPU6050_mstrWrBufPtr - used to store pointer to master write buffer.
*  I2C_MPU6050_mstrWrBufIndex - used to current index within master write
*  buffer.
*  I2C_MPU6050_mstrWrBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 I2C_MPU6050_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = I2C_MPU6050_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(I2C_MPU6050_CHECK_I2C_FSM_IDLE)
        {
            I2C_MPU6050_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = I2C_MPU6050_CHECK_I2C_STATUS(I2C_MPU6050_I2C_STATUS_BUS_BUSY) ?
                            I2C_MPU6050_I2C_MSTR_BUS_BUSY : I2C_MPU6050_I2C_MSTR_NO_ERROR;
        }
        else if(I2C_MPU6050_CHECK_I2C_FSM_HALT)
        {
            I2C_MPU6050_mstrStatus &= (uint16) ~I2C_MPU6050_I2C_MSTAT_XFER_HALT;
                              errStatus  = I2C_MPU6050_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(I2C_MPU6050_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        /* Non-empty for master-slave mode when wakeup enabled */
        I2C_MPU6050_I2CMasterDisableEcAm();

        /* Set up write transaction */
        I2C_MPU6050_state = I2C_MPU6050_I2C_FSM_MSTR_WR_ADDR;
        I2C_MPU6050_mstrWrBufIndexTmp = 0u;
        I2C_MPU6050_mstrWrBufIndex    = 0u;
        I2C_MPU6050_mstrWrBufSize     = cnt;
        I2C_MPU6050_mstrWrBufPtr      = (volatile uint8 *) wrData;
        I2C_MPU6050_mstrControl       = (uint8) mode;

        slaveAddress = I2C_MPU6050_GET_I2C_8BIT_ADDRESS(slaveAddress);

        I2C_MPU6050_mstrStatus &= (uint16) ~I2C_MPU6050_I2C_MSTAT_WR_CMPLT;
        ...
        I2C_MPU6050_SetMasterInterruptMode    (I2C_MPU6050_I2C_INTR_MASTER_MASK);
        ...
        if(I2C_MPU6050_CHECK_I2C_MODE_RESTART(mode))
        {
            I2C_MPU6050_I2C_MASTER_GENERATE_RESTART;
            I2C_MPU6050_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            I2C_MPU6050_TX_FIFO_WR_REG = slaveAddress;
            I2C_MPU6050_I2C_MASTER_GENERATE_START;
        }
    }

    I2C_MPU6050_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterReadBuf
****************************************************************************//**
*
*  Automatically reads an entire buffer of data from a slave device.
*  Once the data transfer is initiated by this function, further data transfer
*  is handled by the included ISR.
*  Enables the I2C interrupt and clears I2C_MPU6050_I2C_MSTAT_RD_CMPLT
*  status.
*
*  \param slaveAddr: 7-bit slave address.
*  \param xferData: Pointer to buffer of data to be sent.
...
*  be ORed together. See I2C_MPU6050_I2CMasterWriteBuf()
*  function for constants.
*
* \return
*  Error status.See I2C_MPU6050_I2CMasterWriteBuf()
*  function for constants.
*
* \globalvars
*  I2C_MPU6050_mstrStatus  - used to store current status of I2C Master.
...
*  I2C_MPU6050_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  I2C_MPU6050_mstrRdBufPtr - used to store pointer to master read buffer.
*  I2C_MPU6050_mstrRdBufIndex - used to current index within master read
*  buffer.
*  I2C_MPU6050_mstrRdBufSize - used to store master read buffer size.
*
*******************************************************************************/
uint32 I2C_MPU6050_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(I2C_MPU6050_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        I2C_MPU6050_state = I2C_MPU6050_I2C_FSM_MSTR_RD_ADDR;
        I2C_MPU6050_mstrRdBufIndex = 0u;
        I2C_MPU6050_mstrRdBufSize  = cnt;
        I2C_MPU6050_mstrRdBufPtr   = (volatile uint8 *) rdData;
        I2C_MPU6050_mstrControl    = (uint8) mode;

        slaveAddress = (I2C_MPU6050_GET_I2C_8BIT_ADDRESS(slaveAddress) | I2C_MPU6050_I2C_READ_FLAG);

        I2C_MPU6050_mstrStatus &= (uint16) ~I2C_MPU6050_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(I2C_MPU6050_mstrRdBufSize < I2C_MPU6050_I2C_FIFO_SIZE)
        {
        ...
        else
        {
            ...
            I2C_MPU6050_ENABLE_MASTER_AUTO_DATA_ACK;
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterSendStart
****************************************************************************//**
*
...
* address byte.
*  - I2C_MPU6050_I2C_WRITE_XFER_MODE - Set write direction for the
*    following transfer.
*  - I2C_MPU6050_I2C_READ_XFER_MODE - Set read direction for the
*    following transfer.
*
* \param timeoutMs
...
* Error status.
*  - I2C_MPU6050_I2C_MSTR_NO_ERROR - Function complete without error.
*  - I2C_MPU6050_I2C_MSTR_BUS_BUSY - Bus is busy.
*    Nothing was sent on the bus. The attempt has to be retried.
*  - I2C_MPU6050_I2C_MSTR_NOT_READY - Master is not ready for to
*    start transfer.
...
*    Nothing was sent on the bus. The attempt has to be retried.
*  - I2C_MPU6050_I2C_MSTR_ERR_LB_NAK - Error condition: Last byte was
*    NAKed.
*  - I2C_MPU6050_I2C_MSTR_ERR_ARB_LOST - Error condition: Master lost
*    arbitration.
*  - I2C_MPU6050_I2C_MSTR_ERR_BUS_ERR - Error condition: Master
*    encountered a bus error. Bus error is misplaced start or stop detection.
*  - I2C_MPU6050_I2C_MSTR_ERR_ABORT_START - Error condition: The start
...
*
*******************************************************************************/
uint32 I2C_MPU6050_I2CMasterSendStart(uint32 slaveAddress, uint32 bitRnW, uint32 timeoutMs)
{
    uint32 errStatus = I2C_MPU6050_I2C_MSTR_NOT_READY;

    if(I2C_MPU6050_CHECK_I2C_FSM_IDLE)
    {
        I2C_MPU6050_DisableInt();  /* Lock from interruption */

        if(I2C_MPU6050_CHECK_I2C_STATUS(I2C_MPU6050_I2C_STATUS_BUS_BUSY))
        {
            errStatus = I2C_MPU6050_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            uint32 locStatus;
            uint32 timeout = I2C_MPU6050_I2C_CONVERT_TIMEOUT_TO_US(timeoutMs);

            /* Non-empty for master-slave mode when wakeup enabled */
            I2C_MPU6050_I2CMasterDisableEcAm();

            /* Clean up the hardware before a transaction. */
            I2C_MPU6050_SetMasterInterruptMode    (I2C_MPU6050_NO_INTR_SOURCES);
            I2C_MPU6050_ClearMasterInterruptSource(I2C_MPU6050_INTR_MASTER_ALL);
            ...
            else             /* Read direction */
            {
                I2C_MPU6050_state = I2C_MPU6050_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= I2C_MPU6050_I2C_READ_FLAG;
            }
            ...
            do
            {
                locStatus  = I2C_MPU6050_GetMasterInterruptSource() & I2C_MPU6050_INTR_MASTER_SEND_BYTE;
                locStatus |= I2C_MPU6050_GetSlaveInterruptSource() & I2C_MPU6050_SLAVE_INTR_I2C_ADDR;
                locStatus |= I2C_MPU6050_I2CMasterWaitOneUnit(&timeout);
            }
            while (0u == locStatus);

            /* Handle completion event and convert to status */
            errStatus = I2C_MPU6050_I2CMasterHandleStatus(locStatus);
        }
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterSendRestart
****************************************************************************//**
*
...
* \param bitRnW
* Direction of the following transfer. It is defined by read/write bit within
* address byte.See I2C_MPU6050_I2CMasterSendStart() function for constants.
*
* \param timeoutMs
* Defines in milliseconds the time that this function can block for.
* If that time expires, the function returns. If a zero is passed,
...
* \return
* Error status.
* See I2C_MPU6050_I2CMasterSendStart() function for constants.
*
* \sideeffect
...
*
*******************************************************************************/
uint32 I2C_MPU6050_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW, uint32 timeoutMs)
{
    uint32 errStatus = I2C_MPU6050_I2C_MSTR_NOT_READY;

    /* Check FSM state before generating ReStart condition */
    if(I2C_MPU6050_CHECK_I2C_MASTER_ACTIVE)
    {
        uint32 locStatus;
        uint32 timeout = I2C_MPU6050_I2C_CONVERT_TIMEOUT_TO_US(timeoutMs);
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterSendStop
****************************************************************************//**
*
...
*
*******************************************************************************/
uint32 I2C_MPU6050_I2CMasterSendStop(uint32 timeoutMs)
{
    uint32 errStatus = I2C_MPU6050_I2C_MSTR_NOT_READY;
    ...
    if(I2C_MPU6050_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = I2C_MPU6050_GetMasterInterruptSource() & I2C_MPU6050_INTR_MASTER_SEND_STOP;
            locStatus |= I2C_MPU6050_I2CMasterWaitOneUnit(&timeout);
        }
        while (0u == locStatus);
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterWriteByte
****************************************************************************//**
*
...
*
*******************************************************************************/
uint32 I2C_MPU6050_I2CMasterWriteByte(uint32 wrByte, uint32 timeoutMs)
{
    uint32 errStatus = I2C_MPU6050_I2C_MSTR_NOT_READY;
    ...
    if(I2C_MPU6050_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        I2C_MPU6050_TX_FIFO_WR_REG = wrByte;
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterReadByte
****************************************************************************//**
*
...
* I2C_MPU6050_I2CMasterSendStop() or
* I2C_MPU6050_I2CMasterSendRestart() function appropriately.
* This function is blocking. It does not return until a byte is received or
* an error occurs.
*
* \param ackNack
* Response to received byte.
*  - I2C_MPU6050_I2C_ACK_DATA - Generates ACK.
*     The master notifies slave that transfer continues.
*  - I2C_MPU6050_I2C_NAK_DATA - Prepares to generate NAK.
...
*
*******************************************************************************/
uint32 I2C_MPU6050_I2CMasterReadByte(uint32 ackNack, uint8 *rdByte, uint32 timeoutMs)
{
    uint32 errStatus = I2C_MPU6050_I2C_MSTR_NOT_READY;
    ...
    if(I2C_MPU6050_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = I2C_MPU6050_GetMasterInterruptSource() & I2C_MPU6050_INTR_MASTER_RECEIVE_BYTE;
            rxNotEmpty = I2C_MPU6050_GetRxInterruptSource() & I2C_MPU6050_INTR_RX_NOT_EMPTY;
            locStatus |= I2C_MPU6050_I2CMasterWaitOneUnit(&timeout);
        }
        ...
        if (0u != rxNotEmpty)
        {
            *rdByte = (uint8)I2C_MPU6050_RX_FIFO_RD_REG;
            I2C_MPU6050_ClearRxInterruptSource(I2C_MPU6050_INTR_RX_ALL);
        }
        ...
        if (I2C_MPU6050_I2C_MSTR_NO_ERROR == errStatus)
        {
            if(I2C_MPU6050_I2C_ACK_DATA == ackNack)
            {
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes that has been transferred with an
*  I2C_MPU6050_I2CMasterReadBuf() function.
*
* \return
...
*  I2C_MPU6050_I2C_MSTAT_ERR_ARB_LOST or
*  I2C_MPU6050_I2C_MSTAT_ERR_BUS_ERROR occurred while read transfer.
*
* \globalvars
*  I2C_MPU6050_mstrRdBufIndex - used to current index within master read
*  buffer.
...
{
    return(I2C_MPU6050_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterGetWriteBufSize
****************************************************************************//**
*
...
{
    return(I2C_MPU6050_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterClearWriteBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterStatus
****************************************************************************//**
*
...
*  - I2C_MPU6050_I2C_MSTAT_RD_CMPLT - Read transfer complete.
*    The error condition status bits must be checked to ensure that
*    read transfer was completed successfully.
*  - I2C_MPU6050_I2C_MSTAT_WR_CMPLT - Write transfer complete.
*    The error condition status bits must be checked to ensure that write
*    transfer was completed successfully.
*  - I2C_MPU6050_I2C_MSTAT_XFER_INP - Transfer in progress.
*  - I2C_MPU6050_I2C_MSTAT_XFER_HALT - Transfer has been halted.
*    The I2C bus is waiting for ReStart or Stop condition generation.
*  - I2C_MPU6050_I2C_MSTAT_ERR_SHORT_XFER - Error condition: Write
*    transfer completed before all bytes were transferred. The slave NAKed
*    the byte which was expected to be ACKed.
*  - I2C_MPU6050_I2C_MSTAT_ERR_ADDR_NAK - Error condition: Slave did
*    not acknowledge address.
*  - I2C_MPU6050_I2C_MSTAT_ERR_ARB_LOST - Error condition: Master lost
*    arbitration during communications with slave.
*  - I2C_MPU6050_I2C_MSTAT_ERR_BUS_ERROR - Error condition: bus error
*    occurred during master transfer due to misplaced Start or Stop
*    condition on the bus.
*  - I2C_MPU6050_I2C_MSTAT_ERR_ABORT_XFER - Error condition: Slave was
...
*  - I2C_MPU6050_I2C_MSTAT_ERR_XFER - Error condition: This is the
...
uint32 I2C_MPU6050_I2CMasterStatus(void)
{
    ...
    status = (uint32) I2C_MPU6050_mstrStatus;

    if (I2C_MPU6050_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) I2C_MPU6050_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterClearStatus
****************************************************************************//**
*
*  Clears all status flags and returns the master status.
*
* \return
*  Current status of master. See the I2C_MPU6050_I2CMasterStatus()
*  function for constants.
*
* \globalvars
*  I2C_MPU6050_mstrStatus - used to store current status of I2C Master.
...
uint32 I2C_MPU6050_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) I2C_MPU6050_mstrStatus;
    I2C_MPU6050_mstrStatus = I2C_MPU6050_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CReStartGeneration
****************************************************************************//**
*
...
void I2C_MPU6050_I2CReStartGeneration(void)
{
    ...
    * In case of timeout the I2C_MPU6050_I2CMasterSendRestart handles this
    * case using own timeout and execute appropriate recovery.
    */

    const uint16 oneUnit = 1u;
    uint32 timeout = I2C_MPU6050_I2C_PHASE_GEN_TIMEOUT;

    /* Store direction of transfer */
    uint32 readDirection = I2C_MPU6050_I2C_STATUS_REG & I2C_MPU6050_I2C_STATUS_M_READ;

    /* Wait until slave releases SDA line: SCL tHIGH is complete */
    while ((I2C_MPU6050_WAIT_SDA_SET_HIGH) && (timeout > 0u))
    {
        CyDelayUs(oneUnit);
        --timeout;
    }
    ...
    {
        /* Refill timeout */
        timeout = I2C_MPU6050_I2C_PHASE_GEN_TIMEOUT;

        /* Drive SCL line use GPIO */
        I2C_MPU6050_SET_I2C_SCL_DR       (I2C_MPU6050_I2C_SCL_LOW);
        I2C_MPU6050_SET_I2C_SCL_HSIOM_SEL(I2C_MPU6050_HSIOM_GPIO_SEL);
        ...
        if (0u != readDirection)
        {
            ...
            I2C_MPU6050_SET_I2C_SCL_DR(I2C_MPU6050_I2C_SCL_LOW);
            CyDelayUs(I2C_MPU6050_I2C_TLOW_TIME);

            /* Drive tHIGH: SCL high phase */
            I2C_MPU6050_SET_I2C_SCL_DR(I2C_MPU6050_I2C_SCL_HIGH);
            CyDelayUs(I2C_MPU6050_I2C_THIGH_TIME);

            /* Wait until slave releases SDA line: SCL tHIGH is complete */
            while ((I2C_MPU6050_WAIT_SCL_SET_HIGH) && (timeout > 0u))
            {
                CyDelayUs(oneUnit);
                --timeout;
            }
        ...
        if (timeout > 0u)
        {
            ...
            I2C_MPU6050_I2C_MASTER_CMD_REG  = I2C_MPU6050_I2C_MASTER_CMD_M_START;
        }
        ...
        */
        ...
        I2C_MPU6050_SET_I2C_SCL_HSIOM_SEL(I2C_MPU6050_HSIOM_I2C_SEL);
        I2C_MPU6050_SET_I2C_SCL_DR       (I2C_MPU6050_I2C_SCL_HIGH);
    }
#else
    uint32 cmd;
    ...
    cmd  = I2C_MPU6050_I2C_MASTER_CMD_M_START;
    cmd |= I2C_MPU6050_CHECK_I2C_STATUS(I2C_MPU6050_I2C_STATUS_M_READ) ?
                (I2C_MPU6050_I2C_MASTER_CMD_M_NACK) : (0u);

    I2C_MPU6050_I2C_MASTER_CMD_REG = cmd;
    ...
    I2C_MPU6050_I2C_MULTI_MASTER_SLAVE_CONST && I2C_MPU6050_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: I2C_MPU6050_I2CMasterDisableEcAm
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    static void I2C_MPU6050_I2CMasterDisableEcAm(void)
    {
        ...
        */
        if (0u != (I2C_MPU6050_CTRL_REG & I2C_MPU6050_CTRL_EC_AM_MODE))
        {
            /* Enable external address match logic */
            I2C_MPU6050_Stop();
            I2C_MPU6050_CTRL_REG &= (uint32) ~I2C_MPU6050_CTRL_EC_AM_MODE;
            I2C_MPU6050_Enable();
        }
    }
#endif /* (!I2C_MPU6050_CY_SCBIP_V0) */
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CMasterHandleStatus
****************************************************************************//**
*
...
* I2C_MPU6050_I2C_MASTER_TIMEOUT and I2C_MPU6050_SLAVE_INTR_I2C_ADDR.
*
* \return
* Error status.
* See I2C_MPU6050_I2CMasterSendStart() function for constants.
...
static uint32 I2C_MPU6050_I2CMasterHandleStatus(uint32 status)
{
    ...
    if (0u != (status & I2C_MPU6050_I2C_MASTER_TIMEOUT))
    {
        retStatus  = I2C_MPU6050_I2C_MASTER_TIMEOUT;
        resetBlock = I2C_MPU6050_I2C_RESET_ERROR;
    }
    else if (0u != (I2C_MPU6050_SLAVE_INTR_I2C_ADDR & status))
    {
        /* Abort the master operation, the slave was addressed first */
        retStatus = I2C_MPU6050_I2C_MSTR_ERR_ABORT_START;

        I2C_MPU6050_I2C_MASTER_CMD_REG = 0u;

        I2C_MPU6050_state = I2C_MPU6050_I2C_FSM_IDLE;
    }
    else if (0u != (status & I2C_MPU6050_INTR_MASTER_I2C_ARB_LOST))
    {
        retStatus  = I2C_MPU6050_I2C_MSTR_ERR_ARB_LOST;

        if (I2C_MPU6050_CHECK_I2C_FSM_ADDR)
        {
            /* If slave is not enabled reset IP, otherwise give it a chance to match address */
            if (0u == (I2C_MPU6050_I2C_CTRL_REG & I2C_MPU6050_I2C_CTRL_SLAVE_MODE))
            {
                resetBlock = I2C_MPU6050_I2C_RESET_ERROR;
            }
    ...
    else if (0u != (status & I2C_MPU6050_INTR_MASTER_I2C_BUS_ERROR))
    {
        retStatus  = I2C_MPU6050_I2C_MSTR_ERR_BUS_ERR;
        resetBlock = I2C_MPU6050_I2C_RESET_ERROR;
    }
    else if (0u != (status & I2C_MPU6050_INTR_MASTER_I2C_NACK))
    {
        retStatus = I2C_MPU6050_I2C_MSTR_ERR_LB_NAK;
    }
    else /* Includes: INTR_MASTER_I2C_STOP and INTR_MASTER_I2C_ACK */
    {
        retStatus = I2C_MPU6050_I2C_MSTR_NO_ERROR;

        if (0u != (status & I2C_MPU6050_INTR_MASTER_I2C_STOP))
        {
        ...
        else
        {
            ...
            if (I2C_MPU6050_CHECK_I2C_FSM_ADDR)
            {
                ...
                I2C_MPU6050_state = (I2C_MPU6050_CHECK_I2C_FSM_RD) ?
                                          I2C_MPU6050_I2C_FSM_MSTR_RD_DATA : I2C_MPU6050_I2C_FSM_MSTR_WR_DATA;
            }
        }
    }

    if (I2C_MPU6050_I2C_RESET_ERROR == resetBlock)
    {
        I2C_MPU6050_I2CFwBlockReset();
    }
    else
    {
        I2C_MPU6050_ClearMasterInterruptSource(I2C_MPU6050_INTR_MASTER_ALL);
...
static uint32 I2C_MPU6050_I2CMasterWaitOneUnit(uint32 *timeout)
{
    ...
    if (*timeout > 0u)
    {
        ...
        if (0u == *timeout)
        {
            status = I2C_MPU6050_I2C_MASTER_TIMEOUT;
        }
    }
