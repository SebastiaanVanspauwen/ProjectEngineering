...
volatile uint8 I2C_MPU6050_state;  /* Current state of I2C FSM */
    ...
    ***************************************/
    ...
    const I2C_MPU6050_I2C_INIT_STRUCT I2C_MPU6050_configI2C =
    {
        ...
        I2C_MPU6050_I2C_BYTE_MODE_ENABLE,
        I2C_MPU6050_I2C_DATA_RATE,
        I2C_MPU6050_I2C_ACCEPT_GENERAL_CALL,
    };
    ...
    *
    *
    *  Configures the I2C_MPU6050 for I2C operation.
    *
    *  This function is intended specifically to be used when the I2C_MPU6050 
    *  configuration is set to “Unconfigured I2C_MPU6050” in the customizer. 
    *  After initializing the I2C_MPU6050 in I2C mode using this function, 
    *  the component can be enabled using the I2C_MPU6050_Start() or 
    * I2C_MPU6050_Enable() function.
    ...
    *******************************************************************************/
    void I2C_MPU6050_I2CInit(const I2C_MPU6050_I2C_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            if (((I2C_MPU6050_I2C_MODE_SLAVE != config->mode) &&
                 (config->dataRate <= I2C_MPU6050_I2C_DATA_RATE_FS_MODE_MAX)) ||
                 (I2C_MPU6050_I2C_MODE_SLAVE == config->mode))
            {
                /* AF = 1, DF = 0 */
                I2C_MPU6050_I2C_CFG_ANALOG_FITER_ENABLE;
                medianFilter = I2C_MPU6050_DIGITAL_FILTER_DISABLE;
            }
            else
            {
                /* AF = 0, DF = 1 */
                I2C_MPU6050_I2C_CFG_ANALOG_FITER_DISABLE;
                medianFilter = I2C_MPU6050_DIGITAL_FILTER_ENABLE;
            }

        #if (!I2C_MPU6050_CY_SCBIP_V0)
            locEnableWake = (I2C_MPU6050_I2C_MULTI_MASTER_SLAVE) ? (0u) : (config->enableWake);
        #else
            locEnableWake = config->enableWake;
        #endif /* (!I2C_MPU6050_CY_SCBIP_V0) */

            /* Configure I2C interface */
            I2C_MPU6050_CTRL_REG     = I2C_MPU6050_GET_CTRL_BYTE_MODE  (config->enableByteMode) |
                                            I2C_MPU6050_GET_CTRL_ADDR_ACCEPT(config->acceptAddr)     |
                                            I2C_MPU6050_GET_CTRL_EC_AM_MODE (locEnableWake);
            ...
            I2C_MPU6050_I2C_CTRL_REG = I2C_MPU6050_GET_I2C_CTRL_HIGH_PHASE_OVS(config->oversampleHigh) |
                    ...
                    I2C_MPU6050_GET_I2C_CTRL_S_GENERAL_IGNORE((uint32)(0u == config->acceptGeneralAddr))    |
                    I2C_MPU6050_GET_I2C_CTRL_SL_MSTR_MODE  (config->mode);

            /* Configure RX direction */
            I2C_MPU6050_RX_CTRL_REG      = I2C_MPU6050_GET_RX_CTRL_MEDIAN(medianFilter) |
            ...
            I2C_MPU6050_INTR_SLAVE_MASK_REG  = ((I2C_MPU6050_I2C_SLAVE) ?
                            (I2C_MPU6050_GET_INTR_SLAVE_I2C_GENERAL(config->acceptGeneralAddr) |
                             I2C_MPU6050_I2C_INTR_SLAVE_MASK) : (I2C_MPU6050_CLEAR_REG));

            I2C_MPU6050_INTR_MASTER_MASK_REG = I2C_MPU6050_NO_INTR_SOURCES;
    ...
    *******************************************************************************/
    void I2C_MPU6050_I2CInit(void)
    {
        ...
        /* Adjust AF and DF filter settings. Ticket ID#176179 */
        I2C_MPU6050_I2C_CFG_ANALOG_FITER_ENABLE_ADJ;
...
/*******************************************************************************
* Function Name: I2C_MPU6050_I2CFwBlockReset
****************************************************************************//**
*
* Resets the scb IP block and I2C into the known state.
*
*******************************************************************************/
void I2C_MPU6050_I2CFwBlockReset(void)
{
    ...
    *******************************************************************************/
    ...
    {
    #if (!I2C_MPU6050_CY_SCBIP_V0)
        #if (I2C_MPU6050_I2C_MULTI_MASTER_SLAVE_CONST && I2C_MPU6050_I2C_WAKE_ENABLE_CONST)
        ...
        #endif /* (I2C_MPU6050_I2C_MULTI_MASTER_SLAVE_CONST) */

        #if (I2C_MPU6050_SCB_CLK_INTERNAL)
            /* Disable clock to internal address match logic. Ticket ID#187931 */
            I2C_MPU6050_SCBCLK_Stop();
    ...
    ****************************************************************************//**
    *
    *  Disables I2C_MPU6050_INTR_I2C_EC_WAKE_UP interrupt source. This interrupt
    *  triggers on address match and wakes up device.
    *
    *******************************************************************************/
