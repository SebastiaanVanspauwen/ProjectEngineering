...
#ifdef I2C_MPU6050_I2C_ISR_ENTRY_CALLBACK
    I2C_MPU6050_I2C_ISR_EntryCallback();
#endif /* I2C_MPU6050_I2C_ISR_ENTRY_CALLBACK */

#if (I2C_MPU6050_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = I2C_MPU6050_I2C_ACK_ADDR;
                ...
                if(I2C_MPU6050_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(I2C_MPU6050_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        ...
                        */
                        if(I2C_MPU6050_CHECK_INTR_RX_MASKED(I2C_MPU6050_INTR_RX_FULL))
                        {
                            ...
                            if(diffCount > I2C_MPU6050_I2C_FIFO_SIZE)
                            {
                                diffCount = I2C_MPU6050_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        ...
                        {
                            while(I2C_MPU6050_I2C_FIFO_SIZE != I2C_MPU6050_GET_TX_FIFO_ENTRIES)
                            {
    ...
    else if(I2C_MPU6050_CHECK_I2C_FSM_SLAVE)
    {
        #if(I2C_MPU6050_I2C_SLAVE)
        {
            ...
            */
            ...
            #ifdef I2C_MPU6050_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                I2C_MPU6050_I2C_SlaveCompleteCallback();
            #endif /* I2C_MPU6050_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
        ...
        #endif /* (I2C_MPU6050_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                #if (I2C_MPU6050_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != I2C_MPU6050_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = I2C_MPU6050_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) I2C_MPU6050_RX_FIFO_RD_REG;
                        response = I2C_MPU6050_I2C_ACK_ADDR;
                    }
            ...
            #if (I2C_MPU6050_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == I2C_MPU6050_I2C_NAK_ADDR)
                {
                #if (!I2C_MPU6050_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    I2C_MPU6050_DISABLE_INTR_SLAVE(I2C_MPU6050_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2C_MPU6050_CY_SCBIP_V0) */
            ...
            #endif /* (I2C_MPU6050_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    I2C_MPU6050_ENABLE_INTR_SLAVE(I2C_MPU6050_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2C_MPU6050_CY_SCBIP_V0) */
            ...
            */
            if(I2C_MPU6050_CHECK_INTR_RX_MASKED(I2C_MPU6050_INTR_RX_FULL))
            {
                ...
                diffCount =  I2C_MPU6050_slWrBufSize -
                            (I2C_MPU6050_slWrBufIndex + I2C_MPU6050_I2C_FIFO_SIZE);

                if(diffCount > I2C_MPU6050_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = I2C_MPU6050_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                while(I2C_MPU6050_I2C_FIFO_SIZE != I2C_MPU6050_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    ...
                    /* Probably Overflow */
                    {
                        ...
                        if(I2C_MPU6050_slOverFlowCount <= I2C_MPU6050_I2C_TX_OVERFLOW_COUNT)
                        {
...
#ifdef I2C_MPU6050_I2C_ISR_EXIT_CALLBACK
    I2C_MPU6050_I2C_ISR_ExitCallback();
#endif /* I2C_MPU6050_I2C_ISR_EXIT_CALLBACK */
