...
void I2CReadBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t *value) {
	...
	I2C_MPU6050_MasterSendRestart(devAddr, I2C_MPU6050_READ_XFER_MODE);
	while (i++ < (length-1)) {
		*value++ = I2C_MPU6050_MasterReadByte(I2C_MPU6050_ACK_DATA);
	}
	*value = I2C_MPU6050_MasterReadByte(I2C_MPU6050_NAK_DATA);
	I2C_MPU6050_MasterSendStop();	
}
...
void I2CWriteBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t *value) {
	...
	I2C_MPU6050_MasterWriteByte(regAddr);
	while (i++ < length) {
		I2C_MPU6050_MasterWriteByte(*value++);
	}
	I2C_MPU6050_MasterSendStop();	
}
...
void I2CWriteWords(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint16_t *value) {
	...
	I2C_MPU6050_MasterWriteByte(regAddr);
	while (i++ < length) {
		I2C_MPU6050_MasterWriteByte(((uint8_t)*value) >> 8);
		I2C_MPU6050_MasterWriteByte((uint8_t)*value++);
	}
	I2C_MPU6050_MasterSendStop();		
}
